...
public enum EnumPipePart implements StringRepresentable {
    ...
    static {
        ...
        HORIZONTALS = fromFacingArray(Direction.NORTH, Direction.SOUTH, Direction.WEST, Direction.EAST);
    }
    ...
    public static EnumPipePart readFromNBT(Tag base) {
        ...
        if (base instanceof StringTag nbtString) {
            String string = nbtString.value();
            return nameMap.getOrDefault(string, CENTER);
        } else if (base instanceof NumericTag numericTag) {
            byte ord = numericTag.byteValue();
            if (ord < 0 || ord > 6) {
                return CENTER;
            }
            return values()[ord];
